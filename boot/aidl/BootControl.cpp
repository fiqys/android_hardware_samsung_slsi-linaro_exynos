...
ScopedAStatus BootControl::getSuffix(int32_t in_slot, std::string* _aidl_return) {
    if (!impl_.IsValidSlot(in_slot)) {
        ...
        const char* suffix = impl_.GetSuffix(in_slot);
        *_aidl_return = suffix ? suffix : "";
    }
    return ScopedAStatus::ok();
}
...
ScopedAStatus BootControl::markBootSuccessful() {
    // Exynos_BootControl::MarkBootSuccessful() takes a slot, unlike the
    // no-arg version on android::bootable::BootControl used upstream.
